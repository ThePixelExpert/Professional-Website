...
must_haves:
  truths:
    ...
    - "Customer upsert operations work via Supabase"
...
3. Implement ALL methods from current database.js using Supabase query builder:
   ...
   **updatePaymentStatus(id, paymentStatus, paymentIntentId):**
   - Similar pattern with update().eq().select()

   **createOrUpdateCustomer(customerData):**
   - Use `.from('customers').upsert({...}, { onConflict: 'email' }).select().single()`
...
Run the smoke test to verify client connectivity: `cd contact-backend && npm run test:supabase`. Should show successful connection.
  ...
  <done>
New database service module created with all methods reimplemented using Supabase query builder.
  </done>
</task>

<task type="auto">
  <name>Task 2: Update database.js wrapper and test backend</name>
  <files>contact-backend/database.js</files>
  <action>
Convert the original database.js to a compatibility wrapper and verify the backend works:
...
3. **Start the backend server** and test endpoints:
   ```bash
   cd contact-backend
   npm start
   ```

4. **Test each database operation via API**:
...
5. **Verify in Supabase Studio** (http://localhost:54323):
   - Orders table should have the test order
   - Customers table should have test@test.com customer

6. If any endpoint fails, check error logs and fix the corresponding method in src/services/database.js.
...
3. Customer upsert works (visible in Supabase Studio)
...
- [ ] Customer records appear in customers table
